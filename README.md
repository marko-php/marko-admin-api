...
Admin API provides authenticated JSON endpoints for the admin panel. It serves admin sections filtered by user permissions, section detail with menu items, and current user profile data (roles, permissions). All responses follow a consistent `{data, meta}` / `{errors}` envelope format. Routes are protected by `AdminAuthMiddleware`.
